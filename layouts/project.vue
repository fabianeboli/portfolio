...
<template>
  <div class="flex bg-gray-400 dark:bg-gray-700">
    <Slide v-if="isMobile" class="fixed mx-2 z-50" :menu-type="MenuType.Projects" />
    <aside v-if="!isMobile"
      class="h-screen w-[200px] border-r-2 border-r-gray-200 dark:border-r-gray-900 shadow-lg shadow-gray-500 dark:shadow-transparent bg-slate-100 dark:bg-slate-800 flex-none">
      <div class="flex items-center justify-center">
        ...
        <NuxtLink to="/">
          <UButton color="emerald" variant="soft">
            <Icon size="1.5em" name="mdi:home" />
          </UButton>
        </NuxtLink>
      </div>

      <ul class="flex flex-col gap-y-5 ml-4">
        <li v-for="project in projectMenu" :key="project.id"
          class="text-main dark:text-white text-lg hover:text-emerald-700 cursor-pointer"
          :class="{ 'font-bold !text-emerald-500': project.id.toString() === $route.params.projectSlug }">
